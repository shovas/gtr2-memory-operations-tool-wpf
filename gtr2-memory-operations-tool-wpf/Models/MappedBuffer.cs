...
CCGEPMonitor is visual debugger for GTR2 Shared Memory Plugin.

MappedBuffer implementation.  Implements writing and reading to/from GTR2 shared memory.
...
namespace Gtr2MemOpsTool.Models
{
    public class MappedBuffer<MappedBufferT>
    {
        ...
        private static void GetHeaderBlock<HeaderBlockT>(BinaryReader sharedMemoryStream, int headerBlockBytes, ref HeaderBlockT headerBlock)
        {
            ...
            headerBlock = Marshal.PtrToStructure<HeaderBlockT>(handleBufferHeader.AddrOfPinnedObject())
                ?? throw new InvalidOperationException("Failed to marshal shared memory to structure.");
        ...
        private void MarshalDataBuffer(bool partial, byte[] sharedMemoryReadBuffer, ref MappedBufferT mappedData)
        {
            if (partial)
            {
                ...
                if ( this.fullSizeBuffer == null ) { 
                    throw new InvalidOperationException("Full size buffer is null. This should not happen if Connect() was called successfully.");
                }
                ...
                mappedData = Marshal.PtrToStructure<MappedBufferT>(handlePartialBuffer.AddrOfPinnedObject())
                    ?? throw new InvalidOperationException("Failed to marshal shared memory to structure.");
